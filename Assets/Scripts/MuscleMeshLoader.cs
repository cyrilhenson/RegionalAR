/// RegionalAR — Muscle Mesh Loader (Phase 4 of mesh-based rendering)
///
/// Loads a companion .mmsh file alongside the .vol. Structurally identical
/// to NerveMeshLoader — same .omsh binary format, same [-0.5, 0.5]³ coord
/// space, same cut-plane sync pattern. Muscle meshes only exist when AI
/// mode found discrete muscle group labels (gluteus, iliopsoas, autochthon).
...
public class MuscleMeshLoader : MonoBehaviour
{
    [Header("Appearance")]
    public Color muscleColor = new Color(0.75f, 0.25f, 0.38f, 1f);  // dark pink
    ...
    public bool TryLoadCompanion(string volPath)
    {
        ...
        string meshPath = Path.ChangeExtension(volPath, ".mmsh");
        _lastAttemptedPath = meshPath;
        if (!File.Exists(meshPath))
        {
            _lastError = "not found";
            Debug.Log($"[RegionalAR][MMesh] No companion muscle mesh at {meshPath}");
            return false;
        }
        try { LoadOMSH(meshPath); Debug.Log($"[RegionalAR][MMesh] Loaded {meshPath}"); return true; }
        catch (Exception e)
        {
            _lastError = $"load err: {e.Message}";
            Debug.LogWarning($"[RegionalAR][MMesh] Failed to load {meshPath}: {e.Message}");
            return false;
        }
    }
    ...
    void LoadOMSH(string path)
    {
        ...
        if (ver != 1) throw new InvalidDataException($"Unsupported .mmsh version {ver}");
        ...
        if (posBytes.Length != nV * 12 || nrmBytes.Length != nV * 12 || idxBytes.Length != nT * 12)
            throw new InvalidDataException("Truncated .mmsh");
        ...
        _mesh = new Mesh { name = "RegionalAR_Muscle" };
        ...
        Debug.Log($"[RegionalAR][MMesh] loaded — {nV:N0} verts, {nT:N0} tris");
    }
    ...
    void EnsureMeshObject()
    {
        ...
        _meshGO = new GameObject("MuscleMesh");
        ...
        Shader sh = Resources.Load<Shader>("MuscleMatte");
        string via = "Resources/MuscleMatte";
        if (sh == null) { sh = Shader.Find("RegionalAR/MuscleMatte"); via = "Shader.Find(RegionalAR/MuscleMatte)"; }
        if (sh == null) { sh = Shader.Find("Unlit/Color");          via = "FALLBACK Unlit/Color"; }
        Debug.Log($"[RegionalAR][MMesh] Muscle shader resolved via: {via} (shader={(sh != null ? sh.name : "NULL")})");

        var mat = new Material(sh);
        if (mat.HasProperty("_Color")) mat.SetColor("_Color", muscleColor);
